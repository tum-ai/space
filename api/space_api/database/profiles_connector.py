...
def create_db_profile_from_auth0_user(
    sql_engine: sa.Engine,
    auth0_user: AuthenticatedUser,
) -> Profile:
    with Session(sql_engine) as db_session:
        print(auth0_user)
        first_name = auth0_user.first_name
        last_name = auth0_user.last_name
...
def retrieve_or_create_db_profile_by_firebase_uid(
    ...
    with Session(sql_engine) as db_session:
        db_model: Profile | None = (
            ...
            .filter(Profile.auth0_uid == auth0_user.user_id)
            .one_or_none()
        )
        if db_model is None:
            return create_db_profile_from_auth0_user(sql_engine, auth0_user)
        else:
            if not db_model:
                raise KeyError
            db_model.force_load()
