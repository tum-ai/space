...
import {
  ...
  BreadcrumbPage,
  BreadcrumbSeparator,
} from "@components/ui/breadcrumbs-components";
...
export default function Breadcrumbs({
  ...
  const breadcrumbs: Breadcumb[] = paths.map((path) => ({
    displayName: path.slice(0, 1).toUpperCase() + path.slice(1),
    path: path,
  }));
      ...
      <BreadcrumbList>
        ...
        {breadcrumbs.map(({ displayName, path }, index) => (
          <React.Fragment key={index}>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink href={findFullPath(path)}>
                {displayName}
              </BreadcrumbLink>
            </BreadcrumbItem>
          </React.Fragment>
