...
from sqlalchemy import String, Integer, ForeignKey, Column, DateTime, func, Boolean, CheckConstraint
from sqlalchemy.orm import Mapped, mapped_column, relationship

from database.db_models import MixinAsDict, Base
...
class CertificationRequest(MixinAsDict, Base):
    ...
    profile: Mapped["Profile"] = relationship("Profile", back_populates="certification_requests")

    template_handle: Mapped[str] = mapped_column(ForeignKey(CertificationTemplate.handle), nullable=False)
    template: Mapped["CertificationTemplate"] = relationship("CertificationTemplate", back_populates="requests")
...
class CertificationFeedback(MixinAsDict, Base):
    ...
    # [MANAGED FIELDS] #################################################################################################
    request_id: Mapped[int] = mapped_column(ForeignKey(CertificationRequest.id, ondelete='SET NULL'), primary_key=True)
    approver_id: Mapped[int] = mapped_column(ForeignKey(Profile.id, ondelete='SET NULL'), primary_key=True)
    ...
    # [RELATIONAL FK FIELDS] ###########################################################################################
    request: Mapped["CertificationRequest"] = relationship("CertificationRequest", back_populates="feedback")
    approver: Mapped["Profile"] = relationship("Profile")

    __table_args__ = (
        (CheckConstraint('(approved) <> (NOT csv_replacors_values IS NULL)', name='handle_xor_link')),
    )
    ...
    def __repr__(self) -> str:
        return f"CertificationFeedback(request_id={self.request_id}, approver_id={self.approver_id}, " \
...
class Certificate(MixinAsDict, Base):
    ...
    issuer_id: Mapped[int] = mapped_column(ForeignKey(Profile.id, ondelete='SET NULL'), nullable=True)
    ...
    template: Mapped["CertificationTemplate"] = relationship("CertificationTemplate", back_populates="certificates")

    request_id: Mapped[int] = mapped_column(ForeignKey(CertificationRequest.id, ondelete='SET NULL'), nullable=True)
    request: Mapped["CertificationRequest"] = relationship("CertificationRequest", back_populates="certificate")
    ...
    def __repr__(self) -> str:
        return f"Certificate(id={self.id}, profile_id={self.profile_id}, " \
               f"issuer_id={self.issuer_id}, template_handle={self.template_handle}, request_id={self.request_id})"
